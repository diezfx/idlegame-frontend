	...
	<div class="flex flex-col gap-4">
		...
		<div class="grid grid-cols-2 max-h-[360px] gap-2 overflow-y-auto">
			{#each equippableItems as item}
				<button
					...
					class="flex flex-col gap-1 rounded-lg border p-3 text-left hover:bg-muted {selectedItem?.id == item.id
						? 'border-primary bg-primary/10'
						: 'border-border'}"
				>
					<div class="text-sm font-medium">{item.id}</div>
					<div class="text-xs text-muted-foreground">Quantity: {String(item.quantity)}</div>
					<div class="mt-1 flex flex-wrap gap-1">
						{#each itemMasterdata.get(item.id)?.effects as effect}
							<span class="rounded bg-secondary px-1.5 py-0.5 text-[10px] text-foreground">
		...
		<div class="rounded-md border border-border p-3">
			...
			<div class="mt-3 grid grid-cols-6 gap-3 items-center">
				<label for="itemAmount" class="col-span-2 text-sm text-foreground">Amount</label>
				<input
					id="itemAmount"
				...
				<div class="col-span-6 text-right text-sm">
					<span class="inline-block rounded bg-secondary px-2 py-0.5">
						{itemAmount}
					</span>
				</div>
			</div>
		...
		<div class="flex justify-end gap-2">
			<button
				class="rounded-md border border-border bg-card px-3 py-2 text-sm text-foreground hover:bg-muted"
