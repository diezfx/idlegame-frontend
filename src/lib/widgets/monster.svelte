		...
		<DescriptionList>
			...
			<DescriptionRow term="Stats">
				<div class="grid grid-cols-4 gap-1 text-[10px]">
					<span class="rounded bg-destructive/10 px-1 py-0.5 text-center">STR {monster.stat?.strength ?? 0}</span>
					...
					<span class="rounded bg-secondary px-1 py-0.5 text-center">VIT {monster.stat?.vitality ?? 0}</span>
				</div></DescriptionRow
			>
		...
		<div class="col-span-2 grid grid-cols-3 justify-items-center gap-2">
			...
			{#each monster.equippedItems as item (item.id)}
				<div>
					...
					{#if item.quantity > 1}
						<span class="text-[9px] text-muted-foreground leading-tight">x{item.quantity}</span>
					{/if}
					{#if itemDeleteAction}
						<button
							class="absolute -right-1.5 -top-1.5 hidden group-hover:flex h-4 w-4 items-center justify-center rounded-full bg-destructive text-[10px] text-primary-foreground hover:bg-destructive shadow-sm z-10"
							onclick={(e) => {
								e.stopPropagation();
					...
					{#if openEquipDialog}
						<button
							class="absolute bottom-0 left-1/2 -translate-x-1/2 hidden group-hover:block hover:text-foreground bg-card/80 rounded-full p-0.5"
							onclick={(e) => {
								e.stopPropagation();
								openEquipDialog?.();
							}}
